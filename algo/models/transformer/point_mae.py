...
class MaskedPointNetEncoder(nn.Module):
    def __init__(self,
        ...
        self.group_divider = Group(num_group=num_group, group_size=group_size).cuda()
        self.masked_encoder = MaskedEncoder(mask_ratio=mask_ratio,
                                            mask_type=mask_type,
    ...
    def forward(self, xyz):
        ...
        # print(f"x_vis shape: {x_vis.shape}")  # Debugging print
        x_concat = x_vis.view(x_vis.size(0), -1)  # Flatten groups: [B, G', C] -> [B, G'*C]
        # print(f"x_concat shape: {x_concat.shape}")  # Debugging print
