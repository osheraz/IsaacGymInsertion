...
from isaacgyminsertion.allsight.experiments.models.train_allsight_regressor.vit_pytorch import MAE
...
class Trainer(object):
    ...
    def __init__(self, params):
        ...
        if params['input_type'] == 'single':
            ...
            self.tac_enc = SimpleViT(image_size=params['image_size'],
                                   patch_size=14,
                                   num_classes=output_map[params['output']],
                                   channels=params['num_channels'],
                                   dim=512,
            ...
            print('Loaded Tactile enc')
            path_to_dir = '/home/roblab20/osher3_workspace/src/isaacgym/python/IsaacGymInsertion/isaacgyminsertion/allsight/experiments/models/train_allsight_regressor/train_history/real/'
            model_name = 'train_pose_force_resnet18_single_08-02-2024_10-13-42'

            self.tac_enc.load_state_dict(torch.load(path_to_dir + model_name + '/model.pth'))
            self.tac_enc.eval()

            self.mae = MAE(encoder=self.tac_enc,
                           masking_ratio=0.5,  # the paper recommended 75% masked patches
                           decoder_dim=512,  # paper showed good results with just 512
                           decoder_depth=6  # anywhere from 1 to 8
                           ).to(device)

            self.model = MLP(units=[512, 256, output_map[params['output']]], input_size=512).to(device)
        else:
    ...
    def prepare_data(self, paths_train, path_test, output_type):
        ...
        df_data_test = df_data_test[df_data_test.time > 2.0]  # train only over touching samples!
        df_data_test = df_data_test.reset_index(drop=True)
        ...
        if self.params['aug']:
            self.augset = TactileDataset(self.model_params, train_df, output_type,
                                         self.aug_transform, half_image=self.half_image, num_channels=self.num_channels
                                          , remove_ref=self.remove_ref)
        ...
        self.validset = TactileDataset(self.model_params, valid_df, output_type, self.test_transform,
                                       half_image=self.half_image, num_channels=self.num_channels
                                       , remove_ref=self.remove_ref)
    ...
    def run_training_loop(self):
        ...
        for epoch in range(epochs):
            ...
            with tqdm(self.trainloader, unit="batch") as tepoch:
                for (batch_x_img, batch_x_ref, batch_y) in tepoch:
                    tepoch.set_description(f"Epoch [{epoch}/{epochs}]")

                    batch_x = self.mae.get_encoding(batch_x_img).to(device)  # batch_x_ref
            ...
            self.log_model_predictions(batch_x_img, pred, true, rmse_train_loss, 'train')
    ...
    def run_validation_loop(self, EVAL_COSTS):
        ...
        self.log_model_predictions(batch_x_img, pred, true, rmse_curr_valid_loss, 'valid')
    ...
    def run_test_loop(self):
        ...
        for b, (batch_x_img, batch_x_ref, batch_y) in enumerate(self.testloader):
            with torch.no_grad():
        ...
        self.log_model_predictions(batch_x_img, pred, true, rmse_curr_test_loss, 'test')
...
def main():
    ...
    path_to_log = os.path.join(os.path.dirname(os.path.realpath(__file__)),
                               'train_history/ENC_{}'.format(params['train_type']))
